...
export class PermisoComponent implements OnInit {
  ...
  getPermiso(): void {
    this.permisoService.getPermiso().subscribe(response => {
...
public onNewPermiso($event): void {
  ...
  if ($event) {
    const permisoForm = this.modalService.open(PermisoNewComponent, {size: 'lg'});
    permisoForm.componentInstance.title = 'Nuevo permiso';
    permisoForm.result.then((result) => {
      this.permisoService.savePermiso(result).subscribe(response => {
        if (response.success) {
          this.getPermiso();
        }
      }, error => {
...
edit(id: number): void {
 console.log("Editado");
  this.permisoService.getPermisoById(id).subscribe(response => {
    this.permiso = response.data;
   // console.log(this.permiso);
    const permisoForm = this.modalService.open(PermisoEditComponent, {size: 'lg'});
    permisoForm.componentInstance.title = 'Editar permiso';
    permisoForm.componentInstance.permiso = this.permiso;
    permisoForm.result.then((result) => {
      if (result) {
        this.permisoService.updatePermiso(this.permiso.id, result).subscribe(resp => {
          if (resp.success) {
            this.getPermiso();
          }
        }, error => {
